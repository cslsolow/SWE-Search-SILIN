...
class ActionAgent(BaseModel):
    ...
    def run(self, node: Node, experience, instructor):
        ...
        try:
            ...
            print(f'the reasoning is {reason},\n the instruction is {instruction} and the type of the hoped action is {ty}.')
            
            completion_response = self._completion.create_completion(
                instruction, system_prompt=system_prompt, response_model=action_args
            )
